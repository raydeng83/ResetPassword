    ...
    for (String pcv : pcvList)
    {
      ...
      try
      {
        ...
        if ((state != null) && ((credentialValidator instanceof ChallengeablePasswordCredential)))
        {
          ...
          {
            for (Map.Entry<?, List<String>> attribute : result.getAttributes().entrySet())
            {
            ...
            if (attemptToRecover)
            {
              if (this.adminAllowsPasswordChanges)
              {
                ...
                else
                {
                  ...
                  ChallengeWithForm form = new ChallengeWithForm.Builder().clientId((String)inParameters.get("com.pingidentity.adapter.input.parameter.oauth.client.id")).spAdapterId((String)inParameters.get("com.pingidentity.adapter.input.parameter.sp.adapter.id")).authnPolicy(authnPolicy).url(resumeUrl).name(username).loginFailed(true).entityId(entityId).errorMessageKey(null).authnMessageKey(loginContext.getMessageKey()).serverError(null).isChainedUsernameAvailable(isChainedUsernameAvailable).pwdCrdVal(this.pwdCrdVal).pwmLocation(this.pwmLocation).allowsChangePassword(this.adminAllowsPasswordChanges).loginTemplateName(this.loginTemplateName).idpAdapterId(this.idpAdapterId).entityId(entityId).localIdentityProfileId(this.localIdentityProfileId).cookieName(this.cookieName).resetType(this.resetType).enableRememberMyUsername(this.enableRememberMyUsername).allowUsernameEdits(this.allowUsernameEdits).enableUsernameRecovery(this.enableUsernameRecovery).resetType(this.resetType).build();
