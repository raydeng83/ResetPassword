...
public class IdentifyServlet extends AbstractPasswordResetServlet
{
  ...
  public void doGet(HttpServletRequest request, HttpServletResponse response)
    throws IOException
  {
    logger.debug("GET Request to /ext/pwdreset/Identify");
...
//      break;
      try {
        TemplateRendererUtil.render(request, response, configuration.getSelectMethodTemplate(), defaultParams);
      } catch (Exception ex) {
    ...
    case RecoverUsername: 
      ...
      redirect(response, HtmlFormIdpAuthnAdapter.getRecoverUsernameUrl(baseUrl, configuration.getAdapterId(), form.getTargetResource()), false);
      break;
    case Cancel: 
      clearState(request, response);
  ...
  private IdentifyForm parseRequest(HttpServletRequest request, HttpServletResponse response) throws InvalidRequestParameterException
  {
  ...
  private void render(HttpServletRequest request, HttpServletResponse response, Map<String, Object> params)
  {
    ...
    try
    {
      com.pingidentity.sdk.template.TemplateRendererUtil.render(request, response, configuration.getUsernameTemplate(), params);
    }
    catch (Exception ex)
    {
      logger.error("Error on Request to /ext/pwdreset/Identify", ex);
...
/* Location:              D:\workhouse\PasswordReset\myformadapter\src\!\com\pingidentity\adapters\htmlform\pwdreset\servlet\IdentifyServlet.class
