...
import com.efx.pingfed.adapters.htmlform.idrecovery.servlet.RecoverUsernameServlet;
import com.efx.pingfed.adapters.htmlform.pwdchange.common.ChangePasswordSessionState;
import com.efx.pingfed.adapters.htmlform.pwdreset.servlet.*;
import com.efx.pingfed.adapters.htmlform.session.HtmlFormSessionStateSupport;
...
import com.pingidentity.common.security.*;
...
public class HtmlFormIdpAuthnAdapter implements IdpAuthenticationAdapterV2, SessionAwareAuthenticationAdapter
{
    ...
    throws IOException
  {
    ...
    String challengeResponse = req.getParameter("pf.challengeResponse");
    String hiddenAdapterId = req.getParameter("pf.adapterId");
    ...
    boolean changePassword = "true".equalsIgnoreCase(req.getParameter("ChangePassword"));
  ...
  private void handleExpiringPasswordWarningCancel(HttpServletRequest req, HttpServletResponse resp, String resumeUrl, HtmlFormLoginContext loginContext)
  {
    ...
    else if ((req.getParameter("pf.pcvId") != null) && (CommonValidator.isValidAdapterInstanceId(req.getParameter("pf.pcvId"))))
    {
      notificationLoginContext = loginContext;
      pcvId = EOLUtil.stripEOLs(req.getParameter("pf.pcvId"));
    }
    ...
    if ((notificationLoginContext != null) && (!Util.isEmpty(notificationLoginContext.getAuthnIds())))
    {
      ...
      if (authenticatedUser != null)
      {
        ...
        if ((StringUtils.isNotBlank(req.getParameter("pf.notificationCancel"))) && (isResetPendingPassword))
        {
          ...
          {
            HtmlFormIdpAuthnAdapterUtils.addCookie("pf-hfa-exp-pwd", pendingPwdCookieValue, this.expiringPasswordSnoozeInterval, resp);
          }
        }
      }
  ...
  {
    return (StringUtils.isNotBlank(req.getParameter("pf.passwordExpiring"))) && 
      (Boolean.TRUE.toString().equals(req.getParameter("pf.passwordExpiring")));
  }
    ...
    throws IOException
  {
    ...
    String username = (String)transactionalStateSupport.removeAttribute("radius-username", req, resp);
    Object state = transactionalStateSupport.removeAttribute("radius-state", req, resp);
    ...
    throws IOException
  {
    ...
    else if (StringUtils.isNotBlank(req.getParameter("pf.alternateAuthnSystem")))
    {
      TransactionalStateSupport transactionalStateSupport = new TransactionalStateSupport(resumeUrl);
      transactionalStateSupport.removeAttribute(this.SESSION_KEY_LOGIN_CONTEXT, req, resp);
      Map<String, Object> attributes = new HashMap();
      attributes.put("policy.action", req.getParameter("pf.alternateAuthnSystem"));
      loginContext.setAuthnIds(attributes);
      loginContext.setUserName(null);
      loginContext.setAlternateAuthnSystem(true);

    }
    else if (Boolean.parseBoolean(req.getParameter("pf.registration")))
    {
      TransactionalStateSupport transactionalStateSupport = new TransactionalStateSupport(resumeUrl);
      transactionalStateSupport.removeAttribute(this.SESSION_KEY_LOGIN_CONTEXT, req, resp);
      Map<String, Object> attributes = new HashMap();
    ...
    else
    {
      ...
      InputValidator.validate("pf.username", username, new ValidationRule[] { new UsernameRule() });
  ...
  static void updateUserNameCookie(HttpServletRequest req, HttpServletResponse resp, String username, boolean isChainedUsernameAvailable, boolean allowUsernameEdits, int rememberUsernameCookieLifetime, String cookieName)
  {
    ...
    {
      boolean rememberUsername = "on".equalsIgnoreCase(req.getParameter("pf.rememberUsername"));
    ...
    throws IOException
  {
    ...
    ChangePasswordSessionState state = new Builder().fromHtmlFormAdapter(true).clientId(StringUtils.isNotBlank(clientId) ? clientId : null).sessionKeyLoginContext(sessionKeyLoginContext).chainedUsernameAvailable(isChainedUsernameAvailable).chainedUsername(chainedUsername).authnMessageKey(authnMessageKey).authnPolicy(authnPolicy).passwordExpiring(isResetPendingPassword(req)).spAdapterId(StringUtils.isNotBlank(spAdapterId) ? spAdapterId : null).build();
    state.save(req, resp);
  ...
  {
    HandlerRegistry.registerHandler("/pwdreset/Identify", new IdentifyServlet());
    HandlerRegistry.registerHandler("/pwdreset/SelectMethod", new SelectMethodServlet());
    HandlerRegistry.registerHandler("/pwdreset/SecurityCode", new SecurityCodeServlet());
    HandlerRegistry.registerHandler("/pwdreset/Success", new SuccessServlet());
  ...
  {
    HandlerRegistry.registerHandler("/pwdchange/Identify", new com.efx.pingfed.adapters.htmlform.pwdchange.servlet.IdentifyServlet());
    HandlerRegistry.registerHandler("/pwdchange/Success", new com.efx.pingfed.adapters.htmlform.pwdchange.servlet.SuccessServlet());
  }
    ...
    throws AuthnAdapterException, IOException
  {
    ...
    AUTHN_STATUS responseStatus = null;
    ...
    String usernameFromRequest = req.getParameter("pf.username");
    ...
    if (StringUtils.isNotBlank(requestedAction))
    {
      ...
      authnAdapterResponse.setAuthnStatus(AUTHN_STATUS.ACTION);
    ...
    {
      responseStatus = AUTHN_STATUS.ACTION;
      username = loginContext.getUserName();
    }
    else if (resp.isCommitted())
    {
      responseStatus = AUTHN_STATUS.IN_PROGRESS;
    }
    else if ((responseAttributeMap == null) || (responseAttributeMap.isEmpty()))
    {
      responseStatus = AUTHN_STATUS.FAILURE;
    }
    else
    {
      responseStatus = AUTHN_STATUS.SUCCESS;
    }
  ...
  {
    return new IdpAuthnAdapterDescriptor(this, "HTML Form IdP Adapter", this.httpFormGuiConfiguration.createAttributeContract(), true, this.httpFormGuiConfiguration
...
/* Location:              D:\workhouse\PasswordReset\myformadapter\src\!\com\pingidentity\adapters\htmlform\idp\HtmlFormIdpAuthnAdapter.class
