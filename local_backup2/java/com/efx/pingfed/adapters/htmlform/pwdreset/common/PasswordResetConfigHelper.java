...
public final class PasswordResetConfigHelper
{
  ...
  private static PasswordManagementConfiguration createPasswordManagementConfiguration(String adapterId)
  {
    ...
    String resetType = "";
    if(isOTPResetAllowed) {
    	resetType+= HtmlFormGuiConfiguration.RESET_TYPE_OTP_VALUE + " ";
    }
    if(isPingIDResetAllowed) {
    	resetType+= HtmlFormGuiConfiguration.RESET_TYPE_PINGID_VALUE + " ";
    }
    if(isSMSResetAllowed) {
    	resetType+= HtmlFormGuiConfiguration.RESET_TYPE_SMS_VALUE + " ";
    }
    resetType = resetType.trim();
    logger.info("...resetType=" + resetType);
    pwdConfig.setResetType(StringUtils.defaultIfEmpty(resetType, "NONE"));
    //
    ...
    int rememberUsernameCookieLifetime = ForgotPasswordHtmlFormIdpAuthnAdapter.getRememberUsernameCookieLifetime(NumberUtils.toInt(configuration.getFieldValue("'Remember My Username' Lifetime"), Integer.parseInt("30")));
